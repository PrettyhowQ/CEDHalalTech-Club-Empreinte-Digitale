...
export function Header() {
  ...
  const navigation = [
    { name: 'Récitateurs Coran 🎧', href: '/lecteur-coran', priority: true, quran: true },
    ...
    { name: 'Expansion Golfe 100% 🏆', href: '/fiqh-expansion', priority: true, premium: true },
    ...
    { name: 'Lecteur Coran 📖', href: '/lecteur-coran', priority: true, premium: true },
    ...
    <header className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-50">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          {/* BOUTON CORAN GÉANT DANS HEADER */}
          <div className="flex items-center">
            <Link href="/lecteur-coran">
              <div className="bg-gradient-to-r from-green-600 to-emerald-700 text-white px-8 py-4 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 mr-6">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center">
                    <span className="text-2xl">🎧</span>
                  </div>
                  <div>
                    <div className="text-xl font-bold">ÉCOUTER CORAN</div>
                    <div className="text-sm opacity-90">Récitateurs authentiques</div>
                  </div>
                </div>
              </div>
            </Link>
            ...
            {navigation.map((item) => (
              <Link
                ...
                className={`px-3 py-2 text-sm font-medium transition-colors ${
                  item.quran
                    ? 'bg-gradient-to-r from-green-600 to-emerald-700 text-white rounded-lg hover:from-green-700 hover:to-emerald-800 shadow-lg font-bold'
                    : item.testing
